  ...
  <div class="card mx-5 mt-12 bg-white border border-gray-300 rounded-xl">
    ...
    <div v-if="!loading">
      ...
      >
        ...
        >
          <template #body="{ data }">
            <avatar :fullname="data.user.name" :size="70"></avatar>
          </template>
        </Column>
        <Column
          field="name"
...
<Column
        ...
        >
          <template #body="{ data }">
            <span
              ...
              :class="[data.user.status ==='active' ? 'bg-green-500 ' : ( data.user.status ===  'inactive' ? 'bg-red-500' : 'bg-gray-500' )]"
        ...
        >
          <template #body="{ data }">
            <span
              class="py-1.5 px-1.5 text-xs rounded-md text-white"
              :class="[data.user.is_admin !== 1 ? 'bg-blue-500 ' : 'bg-green-500 ']"
              >{{ data.user.is_admin !== 1 ? "user" : "admin" }}</span
            >
          </template>
        </Column>
        <Column
        ...
        <Column header="Actions" class="border-b-[1px] text-center">
          <template #body="{ data }">
            <div class="flex gap-3">
              <button
                @click="removeBlacklist(data.id)"
                title="Debloquer ce compte"
              >
