<template>
    <div class="card mx-5 mt-12 bg-white border border-gray-300 rounded-xl">
        ...
        <div class="flex flex-wrap flex-row ">
            ...
            <div v-if="loading" class="flex-shrink max-w-full px-4 w-full mb-6" id="invoice">
                <div class="p-6 bg-white white:bg-gray-800 rounded-lg shadow-lg">
                    <div
                        class="flex justify-between items-center pb-4 border-b border-gray-200 white:border-gray-700 mb-3">
                    ...
                    <div class="py-4 flex flex-col gap-4">
                        <div class="flex justify-end mr-3">
                            <button @click="addRow"
                                class="bg-green-600 flex gap-2 text-center text-white p-2 rounded-md">
                        ...
                        <table class="table-bordered w-full ltr:text-left rtl:text-right text-gray-600">
                                ...
                                <tr v-for="(row, index) in rows" :key="index" class="border-b-2 border-green-500">
                                    <td>
                                        <div class="flex flex-wrap gap-8 m-2 flex-row items-center">
                                            <select @change="findPrice($event, index)" v-model="rows[index].product_id"
                                    ...
                                    <td class="text-center">
                                        <input v-model="rows[index].quantity" @input="totalAmount(index)" type="text"
                                            name="quantity[]" id="product-quantity"
                                    ...
                                    <td class="text-center">
                                        <input v-model="rows[index].price" type="text" @input="totalAmount(index)"
                                            name="price[]" id="product-price"
                                            class="block w-[70%] p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-sm focus:ring-blue-500 focus:border-blue-500" />
                                    </td>
                                    <td class="text-center">
                                        <input v-model="rows[index].amount" type="text" name="amount[]"
                                            id="product-amount"
                                            class="block w-[70%] p-2 text-gray-900 border border-gray-300 rounded-lg bg-gray-50 text-sm focus:ring-blue-500 focus:border-blue-500" />
                                    </td>
                                    <td class="text-center">
                                        <button @click="deleteRow(index)">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="w-7 h-7 text-red-800">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <tfoot>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center font-bold">{{ common.formatNumber(total) }} DH</td>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
            ...
            <div v-else class="flex-shrink max-w-full px-4 w-full mb-6" id="invoice">
                <div class="p-6 bg-white white:bg-gray-800 rounded-lg shadow-lg">
                    ...
                    <div class="py-4 overflow-x-auto">
                        <table class="table-bordered w-full text-gray-600 white:text-gray-400">
                            <thead class="border-b white:border-gray-700">
                                <tr
                                    class="bg-green-800 h-9 text-center text-white white:bg-gray-900 white:bg-opacity-20">
                                    <th>Products</th>
                                    <th class="text-center">Qty</th>
                                    <th class="text-center">Unit price</th>
                                    <th class="text-center">Amount</th>
...
function getInvoice() {
  ...
  }).catch().finally(() => loading.value = true);
}
...
const total = computed(() => {
  ...
  }, 0).toFixed(2); // Ensure that you format the result to two decimal places
});
</script>
