    ...
    <div class="card mt-12 m-6 bg-white border border-gray-300 rounded-xl">
        ...
        <div v-if="!loading">
            <DataTable :value="filteredProducts" removableSort :paginator="true" class="p-datatable-sites -mt-5 m-6"
                ...
                <Column field="description" header="Description" class="border-b-[1px] text-center" sortable>
                    <template #body="{ data }">
                        <span> {{ data.description }}</span>
                    </template>
                </Column>
                <Column field="category.category_name" header="Categorie" class="border-b-[1px] text-center" sortable>
                    <template #body="{ data }">
                        <span> {{ data.category.category_name }} </span>
                    </template>
                </Column>
                <Column field="price" header="Prix" class="border-b-[1px] text-center" sortable>
                    <template #body="{ data }">
                        <span> {{ common.formatNumber(data.price) }} DH</span>
                    </template>
                </Column>
                <Column sortable class="border-b-[1px] text-center" field="quantity" header="Quantite"></Column>
                <Column sortable field="discount" class="border-b-[1px] text-center" header="Coupons">
                    <template #body="{ data }">
